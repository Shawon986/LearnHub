...
export function PageLoader({ label = "Loading your learning space…" }: { label?: string }) {
  return (
    <div
      className="relative flex min-h-[55vh] w-full flex-col items-center justify-center gap-6 overflow-hidden px-4 py-8 sm:gap-10"
      role="status"
      aria-live="polite"
      aria-label={label}
    >
      ...
      {/* Stage: brain + orbits + synapses (perspective gives the rings depth).
          ...
          or landscape phones. vh (not dvh) keeps it stable while the
          mobile URL bar animates during refresh. */}
      <div className="relative h-[min(76vw,42vh,340px)] w-[min(76vw,42vh,340px)] [perspective:900px]">
